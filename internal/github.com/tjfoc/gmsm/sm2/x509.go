...
func (c *Certificate) CheckSignature(algo SignatureAlgorithm, signed, signature []byte) error {
	return checkSignature(algo, signed, signature, c.SubjectKeyId)
}

// CheckSignature verifies that signature is a valid signature over signed from
// a crypto.PublicKey.
func checkSignature(algo SignatureAlgorithm, signed, signature, ski []byte) (err error) {
	//func checkSignature(algo SignatureAlgorithm, signed, signature []byte, ski []byte) (err error) {
	//var hashType Hash
	//
	//switch algo {
	...
	//
	//keyLabel := "SM2SignKey" + string(ski)
	//
	//r, s := sig.R.Bytes(), sig.S.Bytes()
	//
	//ns := netsign.NetSign{}
	//ip, port, password := ns.FindPKCS11Lib()
	//fmt.Printf("x509 ip [%s]\n",ip)
	//fmt.Printf("x509 port [%d]\n",port)
	//fmt.Printf("x509 password [%s]\n",password)
	//socketFd, ret := ns.OpenNetSign(ip, password, port)
	//if ret != 0 {
	//	panic("open netsign error")
	//}
	//
	//ret = ns.Verify(socketFd, 1, digest, bytesCombine(r, s), keyLabel, "sm3")
	//ns.CLoseNetSign(socketFd)
	//if ret == 0 {
	//	return
	//} else {
	//	panic("msp certificate chain verify error")
	//}
	//

	//ns := cncc.NetSign{}
	//// 获取 ip port password
	//ip, port,password := cncc.FindPKCS11Lib()
	//socketFd, _ := ns.OpenNetSign(ip,password,port)
	//verify := ns.Verify(socketFd, 0, digest, signature,  fmt.Sprintf("SM2SignKey%s", ski), "SM3")
	//ns.CLoseNetSign(socketFd)
	//if verify==0{
	//	return nil
	//}else{
	//	panic("msp setup verify sgnature error")
	//}
...
func (c *CertificateRequest) CheckSignature() error {
	return checkSignature(c.SignatureAlgorithm, c.RawTBSCertificateRequest, c.Signature, []byte("test"))
}
...
func CreateCertificateToMem(template, parent *Certificate, pub, signer interface{}) ([]byte, error) {
	der, err := CreateCertificate(rand.Reader, template, parent, pub, signer)
	if err != nil {
		return nil, err
	}
	block := &pem.Block{
