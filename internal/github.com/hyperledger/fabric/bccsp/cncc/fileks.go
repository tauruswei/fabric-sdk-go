...
func (ks *fileBasedKeyStore) StoreKey(k bccsp.Key) (err error) {
	...
	case *gmsm2PrivateKey:
		err = ks.storePrivateKey(hex.EncodeToString(k.SKI()), string(k.SKI()))
		if err != nil {
...
func (ks *fileBasedKeyStore) searchKeystoreForSKI(ski []byte) (k bccsp.Key, err error) {
	...
	for _, f := range files {
		...
		switch key.(type) {
		case *sm2.PrivateKey:
			k = &gmsm2PrivateKey{ski, gmsm2PublicKey{ski, &key.(*sm2.PrivateKey).PublicKey}}
		default:
			continue
		}
...
// 接口保存的是私钥的  ski
func (ks *fileBasedKeyStore) storePrivateKey(alias string, ski string) error {

	// 临时目录 只是为了测试用，生产的时候，keypath 不会为空
	if ks.path == "" {
		ks.path = filepath.Join(os.TempDir(), "keystore-test")
		os.MkdirAll(ks.path, 0755)
		logger.Warningf("keystore path [%s]", ks.path)
	}
	if err := SaveSKI(ks.path, ski); err != nil {
